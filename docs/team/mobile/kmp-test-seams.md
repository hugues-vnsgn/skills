...
That is why the cheapest task that covers the seam is the right one to run: `jvmTest` for pure common logic, and a simulator or device task — `iosSimulatorArm64Test`, `testDebugUnitTest` — only before claiming a platform-touching slice green.
...
No. Run `/tdd` — it is still the loop and the reference for what a good test is. This one layers underneath it, the same way `kmp-module-setup` layers underneath ordinary module design.
