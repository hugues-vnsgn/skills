...
- **The issue trackers.** `gh issue list --repo osxsystem/skills --search "<skill-name>" --state all`, and the same against `--repo mattpocock/skills` for any skill inherited from upstream — that tracker holds years of questions this fork never saw. Fork-local skills (`setup-osxsystem-skills`, everything under `team/`) exist only here, so search only here for those. A question filed twice is a question the page owes an answer to.
