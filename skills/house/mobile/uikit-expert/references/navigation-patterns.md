...
```swift
// ✅ Insert VC behind current without visible animation
...
**Thread safety is non-negotiable.** All UIKit navigation calls must execute on the main thread. Dispatching from a background queue causes undefined behavior, including corrupted navigation bars, blank screens, or hard crashes.
...
// ✅ iOS 14+: Display mode options: .default, .generic ("Back"), .minimal (chevron only)
...
Note that `presentationControllerShouldDismiss` and `presentationControllerDidAttemptToDismiss` are **never called for programmatic `dismiss(animated:)`**; they apply only to user-initiated swipe dismissal.
...
The accessory exposes its current state via the `tabAccessoryEnvironment` trait, `.regular` when expanded and `.inline` when compact, so you can adapt your layout accordingly.
