...
Apple quietly shipped automatic observation tracking in **iOS 18** as an opt-in feature, then made it the default in iOS 26. When enabled, UIKit wraps key lifecycle methods in Swift Observation tracking contexts. Any `@Observable` property read during those methods registers a dependency; when that property changes, UIKit automatically invalidates the relevant method.
...
On macOS 15+, the equivalent key is `NSObservationTrackingEnabled`. **On iOS 26, the key is ignored** because tracking is always on.
