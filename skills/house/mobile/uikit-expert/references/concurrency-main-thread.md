...
- [ ] No `Task.detached` for UI work; use `Task {}` to inherit MainActor isolation
- [ ] Explicit `@MainActor` on closures or `MainActor.run` when hopping back from background
- [ ] No `DispatchQueue.main.sync` from background (deadlock); use `await MainActor.run` instead
