...
- Use `NSLayoutConstraint.activate([])` instead of individual `.isActive = true`
- Create constraints once, then toggle `isActive` or modify `.constant` rather than removing and recreating them
...
- Use `UICollectionView.CellRegistration`: no string identifiers or manual casting
...
```swift
// Store the registration once; creating it inside the cell provider crashes on iOS 15+.
...
- Use `setViewControllers(_:animated:)` for deep links instead of sequential push calls
...
- `UIView.animate` for simple one-shot animations; check `finished` in completion
- `UIViewPropertyAnimator` for gesture-driven, interruptible animation; respect its state machine (inactive → active → stopped)
- `CABasicAnimation` for layer-only properties (cornerRadius, shadow, 3D transforms); set model value first
...
- `UIViewController` is `@MainActor`, so all subclass methods are implicitly main-actor
- Store `Task` references and cancel in `viewDidDisappear`, not `deinit`
- Check `Task.isCancelled` before UI updates after `await`
- `Task.detached` does NOT inherit actor isolation; explicit `MainActor.run` is needed for UI
...
- Downsample with ImageIO at display size instead of loading the full bitmap and resizing
...
One gate per domain: verify these correctness items (details in the matching Core Guideline above):
