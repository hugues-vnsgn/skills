...
Every skill in `engineering/`, `productivity/`, or a `team/<domain>/` folder has a human-facing **docs page** at `docs/<bucket>/<skill-name>.md`, where `<bucket>` is the skill's path under `skills/` — the docs tree mirrors the promoted folders (so `skills/team/mobile/<name>` → `docs/team/mobile/<name>.md`). It is published at `https://aihero.dev/skills-<skill-name>`; the URL is always `skills-<skill-name>` regardless of bucket, so the docs path is repo organisation only — the `team/` tree is fork-local, so aihero.dev never actually hosts those pages, but they live at the same tree path and follow the same template. The page is not the skill and not a copy of `SKILL.md`. Only these buckets are promoted; the rest (`misc/`, `in-progress/`, `deprecated/`) ship no docs page.
...
- **The issue trackers.** `gh issue list --repo hugues-vnsgn/skills --search "<skill-name>" --state all`, and the same against `--repo mattpocock/skills` for any skill inherited from upstream — that tracker holds years of questions this fork never saw. Fork-local skills — everything under `team/` — exist only here, so search only here for those. A question filed twice is a question the page owes an answer to.
