...
A skill this fork does not ship carries `metadata.internal: true` in its `SKILL.md` frontmatter, which keeps it out of the installer's picker while `--skill=<name>` still installs it and `scripts/link-skills.sh` still links it locally. That covers every skill under `skills/team/in-development/` and anything in `deprecated/`. Upstream's `misc/` and `in-progress/` are **not** marked: they are upstream's deliberate published output, so they stay installable and the picker pools them under its own `Other` heading.
