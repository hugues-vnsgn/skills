...
Step 1 depends on `docs/agents/issue-tracker.md`, which [setup-osxsystem-skills](../../skills/team/platform/setup-osxsystem-skills/SKILL.md) writes. Without it the axis still works if you hand it a path. With no spec at all, the Spec sub-agent is skipped and the report says "no spec available" rather than inventing requirements.
