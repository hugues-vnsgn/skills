...
Before using the trace commands below, set `SKILL_DIR` to the absolute path of this skill's directory (the folder containing this `SKILL.md`). The scripts and references live beside it, not in the app under review.
...
3. **Run the main analysis** (with or without `--window`):
   ...
   ```
4. **Interpret with `references/trace-analysis.md`** using these key diagnostics:
