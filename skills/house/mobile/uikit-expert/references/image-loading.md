...
The `CGImageSourceCreateThumbnailAtIndex` API from the ImageIO framework is the gold-standard technique for memory-efficient image loading. Unlike `UIImage`-based resizing, which decompresses the full bitmap first and then scales it, ImageIO uses **streaming I/O** to read just enough data to produce a thumbnail at the target size, never touching the full-resolution bitmap.
