# Research Report — Agent 2: Compose Multiplatform UI Development & CocoaPods Integration

Sources: official kotlinlang.org multiplatform docs (fetched 2026-08-06). One assigned URL, `compose-multiplatform-navigation-routing.html`, returned 404 — its content now lives at `compose-navigation-routing.html`, which was fetched instead. Supplementary official pages fetched: compatibility/versioning, SwiftUI integration, UIKit integration, ViewModel, iOS accessibility, Hot Reload.
...
For a team building shared Kotlin + Compose Multiplatform apps for Android/iOS, these agent skills would pay off:

1. **`kmp-project-scaffold`** — user-invoked. Generates/validates the KMP + CMP project skeleton: module layout, source sets, per-platform entry points (`MainActivity`, `MainViewController`), version catalog pinned to a known-good CMP↔Kotlin↔AGP matrix. Encodes the CMP→Jetpack Compose version table so upgrades don't guess.
2. **`cocoapods-integration`** — model-invoked. Owns the `kotlin("native.cocoapods")` setup: `cocoapods {}` block authoring, `pod()` dependency declarations with the cinterop gotchas (`-fmodules`, `moduleName`, `headers`), Podfile wiring, and the `./gradlew podInstall` → `.xcworkspace` → disable-script-sandboxing workflow. Should embed the troubleshooting table above, since these errors are cryptic and recurrent.
3. **`compose-ios-interop`** — model-invoked. Templates for both interop directions: `ComposeUIViewController` + `UIViewControllerRepresentable`, `UIKitView`/`UIKitViewController` with factory/update patterns, the `CADisableMinimumFrameDurationOnPhone` Info.plist requirement, and `@ObjCAction` callback wiring.
4. **`cmp-resources-and-navigation`** — model-invoked. Enforces `composeResources/` layout, `Res` usage, qualifier conventions, and Navigation Compose with type-safe `@Serializable` routes; knows the `org.jetbrains.androidx.*` coordinates and the non-JVM `viewModel { }` initializer requirement.
5. **`ios-release-verification`** — user-invoked checklist skill: build release configuration on a real device, run XCTest accessibility audit, check VoiceOver traversal, confirm ProMotion frame pacing — catching the debug-build-performance and accessibility traps before TestFlight.
6. Extend the existing **`html-design-to-compose`** skill (already in this repo) with an iOS-verification step, since CMP renders identically via Skia — a simulator screenshot check on iOS complements its Android/desktop parity loop.

Cross-cutting note for all skills: prefer the CocoaPods docs' explicit warning that CocoaPods and direct (`embedAndSignAppleFrameworkForXcode`) integration are mutually exclusive — a scaffold/setup skill should ask which route the team uses and refuse to mix them.
