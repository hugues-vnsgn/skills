...
One file is now restored, and the reason has nothing to do with plugins. The skills.sh installer ([vercel-labs/skills](https://github.com/vercel-labs/skills)) renders its interactive picker as a flat list unless skills carry a group, and it derives groups from exactly one source: `getPluginGroupings()`, which reads `.claude-plugin/marketplace.json` (or `plugin.json`). With a manifest present it renders collapsible headings, each with a "select all" row, which is the difference between ticking 35 skills one at a time and ticking six domains. `plugin.json` can only ever express a single group, so grouping requires `marketplace.json` specifically.
