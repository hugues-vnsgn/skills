...
`kmp-module-setup` scaffolds or audits a shared Kotlin Multiplatform module: which targets to declare, how the source-set hierarchy works, what goes in the version catalog, and how to configure the iOS framework block. It treats the version catalog as a compatibility contract, so Kotlin, AGP, and Compose Multiplatform are pinned and bumped together, never independently.
...
The recurring fork is **expect/actual vs interface + DI**: expect/actual only for leaf utilities (UUID, clock, platform name); interfaces with DI for anything with behavior, state, or that tests must fake. expect/actual *classes* stay off the table, since they are still Beta and officially discouraged.

## Common questions

**Swift is seeing type names like `Kotlinx_coroutines_coreFlow`. Where does that come from?**

A dependency declared with `implementation` instead of `api`. Types from an `implementation` dependency aren't part of your module's public ABI, so the framework header can't name them properly and they surface mangled. Exporting a type across the framework boundary needs `api(...)` in the dependency block *and* `export(...)` in the framework block, both of them, not either one. Avoid reaching for `transitiveExport = true` to make the problem go away; it exports the whole graph and bloats the binary.

**Do I need to wire `dependsOn` between source sets?**

No, for any standard layout. Declaring `androidTarget()`, `iosArm64()`, and `iosSimulatorArm64()` gives you the default hierarchy free on Kotlin 1.9.20 and later, where `commonMain → appleMain → iosMain → iosArm64Main` is already connected. Manual `dependsOn` wiring is the most common thing to find in an audit and delete. You only need it for a genuinely non-standard hierarchy.
