# Publishing Kotlin Multiplatform Apps & Libraries — Research Report (Agent 3/3)

Researched 2026-08-06 against the official kotlinlang.org multiplatform docs plus supporting sources. Target audience: a mobile team shipping shared Kotlin + Compose Multiplatform apps to Android and iOS, and possibly publishing shared libraries to Maven Central.

Primary sources:

- [Publish your application](https://kotlinlang.org/docs/multiplatform/multiplatform-publish-apps.html)
- [Publish your library to Maven Central](https://kotlinlang.org/docs/multiplatform/multiplatform-publish-libraries-to-maven.html)
- [Set up multiplatform library publication](https://kotlinlang.org/docs/multiplatform/multiplatform-publish-lib-setup.html)
- [Test your multiplatform app](https://kotlinlang.org/docs/multiplatform/multiplatform-run-tests.html)
- [GitHub Actions for KMP](https://kotlinlang.org/docs/multiplatform/github-actions-for-kmp.html)

All four assigned pages fetched successfully; no fetch failures.
...
Skills a team in this area would benefit from having (each = one focused agent skill with checklists + project-specific values baked in):

1. **`kmp-release-android`** — Drive an Android release from a KMP repo: bump version, `bundleRelease` with signing from env/secrets, verify R8 output (run minified build, check `missing_rules.txt`), upload to Play internal track via Fastlane supply. Encodes the KMP-specific R8 knowledge (consumer keep rules via `androidLibrary.optimization`, serialization/Ktor keep rules for shared code).
2. **`kmp-release-ios`** — Archive & ship the iOS app: verify `embedAndSignAppleFrameworkForXcode` wiring, `Config.xcconfig` bundle/app-name settings, privacy manifest coverage for the shared framework, dSYM upload, TestFlight upload via Fastlane pilot with an ASC API key. Knows bitcode is dead and where KMP-specific rejections (privacy manifest) come from.
3. **`kmp-publish-maven-central`** — End-to-end library publishing via the Central Portal: vanniktech plugin config, POM requirements, GPG key generation/upload (`generatePgpKeys`, `uploadPublicPgpKey`), the `checkSigningConfiguration`/`checkPomFileForMavenPublication` pre-flight, the single-macOS-host rule, `--no-configuration-cache`, and the release-triggered publish workflow.
4. **`kmp-ci-setup`** — Scaffold/tune GitHub Actions for a KMP repo: composite gradle-setup action, Ubuntu-vs-macOS job split (JVM tests on Ubuntu, link/simulator on macOS), `~/.konan` + Gradle caching keyed on the version catalog, secrets inventory (keystore, ASC key, GPG, Central token).
5. **`kmp-test-matrix`** — Know which Gradle task tests what (`allTests`, `jvmTest`, `iosSimulatorArm64Test`, `testDebugUnitTest`), where reports land, when a test belongs in `commonTest` vs `androidHostTest`/`iosTest`, and how to run simulator tests locally and in CI.
6. **`kmp-library-versioning`** — Enforce the versioning invariants: tag ⇔ Gradle version, binary-compatibility-validator (incl. klib ABI) in CI, semver decisions on common API changes, Kotlin-version coupling notes in release notes, XCFramework/SPM release kept in lockstep with the Maven release.

Highest leverage first: **#4 (CI setup)** and **#2 (iOS release)** — that's where KMP diverges most from what a mobile team already knows; Android publishing (#1) is mostly standard Android with a thin KMP layer.
