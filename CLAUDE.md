...
- `house/mobile/` holds the Kotlin Multiplatform / Compose Multiplatform and native iOS UI skills
...
Upstream encodes maturity in the bucket, where `in-progress/` is beta. A house domain encodes capability instead, so maturity rides in the catalog: a skill whose [.fork/catalog.yaml](./.fork/catalog.yaml) entry carries `status: beta` is exempt from promotion. It gets no docs page, stays out of the top-level `README.md`, and is listed under a flat `## Beta` heading in its bucket `README.md`. Drop the field to promote it, and add the missing registration in the same change.
