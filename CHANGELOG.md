...
- [`94add64`](https://github.com/hugues-vnsgn/skills/commit/94add643104e33754323cb0e48f59d323b6af294) Thanks [@osxsystem](https://github.com/osxsystem)! - Add `swiftui-expert-skill` to the mobile domain for native SwiftUI implementation, review, accessibility, and Instruments trace diagnosis. Vendor the complete skill, references, assets, and trace scripts from avdlee/swiftui-agent-skill at commit `b24e68a965dc4b5bd2cc41dc60c094a26a9379ce` under its MIT license, and register it in the catalog, mobile README, role pages, and router.

- [`94add64`](https://github.com/hugues-vnsgn/skills/commit/94add643104e33754323cb0e48f59d323b6af294) Thanks [@osxsystem](https://github.com/osxsystem)! - Add `uikit-expert` to the mobile domain for native UIKit implementation and review. Vendor the skill and references from ivan-magda/uikit-expert-skill at commit `45c70f0f31e63c62bcb11da6c7bb3b4759dac393` under its MIT license, and register it in the catalog, mobile README, engineer role, and router.

### Patch Changes

- [#19](https://github.com/hugues-vnsgn/skills/pull/19) [`d75dcf1`](https://github.com/hugues-vnsgn/skills/commit/d75dcf1c5bccfab736ab79f494fa248980546d45) Thanks [@hugues-vnsgn](https://github.com/hugues-vnsgn)! - Add the `pr` skill (in-progress bucket, model-invoked). It's a reference for the shape a pull request body should take, not a workflow: the template comes first, then a short section per part of it. The summary comes from the primary source (the issue or spec), never inferred from the diff; the body states size and a one-way/two-way door call up front; "the shape of the change" reproduces `show-me` almost verbatim (credited in the skill's `CREDITS.md`), aimed at a diff instead of a conversation; evidence is a before/after pair (visual first, a failing-then-passing test run where no visual exists); and what was deliberately left out gets its own section. Relates to [#521](https://github.com/hugues-vnsgn/skills/issues/521), [#938](https://github.com/hugues-vnsgn/skills/issues/938), [#509](https://github.com/hugues-vnsgn/skills/issues/509), and [#915](https://github.com/hugues-vnsgn/skills/issues/915).

- [#24](https://github.com/hugues-vnsgn/skills/pull/24) [`3eb6b0d`](https://github.com/hugues-vnsgn/skills/commit/3eb6b0d453f13dec695472feab6d4b3d199e63f2) Thanks [@hugues-vnsgn](https://github.com/hugues-vnsgn)! - Separate mobile skill responsibilities and remove duplicate setup/iOS references. Restrict Ktor bearer credentials to the configured API origin, bound retries, and verify examples with executable fixtures. Refresh Android plugin/test guidance, release scope, docs and routing while preserving beta status.

- [#19](https://github.com/hugues-vnsgn/skills/pull/19) [`6942bff`](https://github.com/hugues-vnsgn/skills/commit/6942bff8a53666e83a2c621696d2a8e3022429e6) Thanks [@hugues-vnsgn](https://github.com/hugues-vnsgn)! - retro: classify coding-standards findings as mechanical or judgement calls before writing them. A mechanical violation (a fixed syntactic pattern, a banned API, an import shape, a file-location rule) now gets a deterministic check instead (a linter rule, a pre-commit hook, or a CI job), reserving `CODING_STANDARDS.md` for genuine judgement calls. Automated checks also now flags a repo with no guardrail at all (no pre-commit hook, no CI lint/typecheck/test job) as a finding in its own right.

- [#21](https://github.com/hugues-vnsgn/skills/pull/21) [`9070a69`](https://github.com/hugues-vnsgn/skills/commit/9070a69623897f2189ba9aa64b6789bdfecc235b) Thanks [@hugues-vnsgn](https://github.com/hugues-vnsgn)! - Rewrite `ddd` from 3,635 words to about 1,240, with the body at or under 1,200, and promote it from `house/in-development/` to `house/discovery/` as a user-invoked skill, with a docs page, README entries, the engineer, analyst, and staff role pages, and an `ask-matt` on-ramp. Two entries, an idea or a repo to redesign, share one crunch report, every line carrying its evidence (a `path:line`, or the user's quoted words): Known, Fractured, Unknown, and a new Implicit list for rules the code enforces or the user assumes without naming. A redesign also draws today's context map beside the target. The conversation keeps the Discovery Question template and the fixed branch order, but checkpoints now ride in the next round, a one-line status strip replaces the ledger table, and one stop question inside round 3 replaces the up-front choice between a strategic and a full pass, so the short path is three replies on a repo and four on an idea. A recommendation that rests on a business fact only the user holds is flagged as a guess. The routing block, the definitions, and the prose defending the design are gone. `ARTIFACTS.md` keeps the notation and trims the pattern and stereotype definitions to name lists.

- [#22](https://github.com/hugues-vnsgn/skills/pull/22) [`51f6c98`](https://github.com/hugues-vnsgn/skills/commit/51f6c98420f3337b752b3b1dc572645dca4897d1) Thanks [@hugues-vnsgn](https://github.com/hugues-vnsgn)! - Promote `tdd-kmp` to the Mobile skills, with test-first guidance across shared Kotlin, Android, Compose, and SwiftUI. Route `/implement` through it for KMP/CMP apps when installed; keep `/tdd` for other code and a KMP test-seam fallback for selective installs.
...
- [#13](https://github.com/hugues-vnsgn/skills/pull/13) [`47de6a3`](https://github.com/hugues-vnsgn/skills/commit/47de6a3c8f9f788f9593db322d30e9b0927cba9b) Thanks [@hugues-vnsgn](https://github.com/hugues-vnsgn)! - Add `ddd`, a skill that runs a domain-driven design session end to end, landing in `house/in-development/` as beta.
