...
The `@MainActor` annotation is crucial here. Because unstructured `Task { }` inherits the caller's actor context, both the task body and the call site access the same `tasks` dictionary on the main actor, **with no data races**. Apple demonstrated this exact pattern at WWDC 2021 ("Explore Structured Concurrency in Swift").
...
let cellRegistration = UICollectionView.CellRegistration<PhotoCell, Photo.ID> {
    ...
    if let image = self.imageCache[photoID] {
        ...
        self.downloadImage(for: photo) {
            // When download completes, reconfigure; do NOT mutate the cell directly
...
Manual frame layout in `layoutSubviews` is **O(1)** with zero constraint-solving overhead, making it the right choice for your highest-frequency cells.
