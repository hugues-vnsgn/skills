...
A manual `EnvironmentKey` with `static let defaultValue` is a deliberate stability fix (evaluated once). Do not use `static var defaultValue: T { Model() }`; it re-evaluates on every fallback read, the same problem `@Entry` has with an inline allocation.
