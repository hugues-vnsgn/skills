...
**Status:** Proposed. Plan 1 of 5 (validation infra) shipped in PR #5; plans 2–5 pending.
...
5. **[2026-08-11] The second-pass adoptions** — `port-from-repo`, `when-stuck`, then the single `ask-matt` re-sync covering all six promoted skills, and `scripts/link-skills.sh`.

Plan 5 goes last for one reason: `port-from-repo` delegates its build phase to `/tdd` and its close-out to `/code-review`, and `ship` (plan 3) is what those hand off to. Writing it before the tail exists would encode a handoff to a skill that isn't there yet. The `ask-matt` re-sync moves out of plan 4 and into plan 5 so the router is still written once, now covering six skills instead of five.
