# Writing

House skills for prose a human reads. The other half of the pair is upstream's [`writing-for-agents`](../../productivity/writing-for-agents/SKILL.md), which covers documents an agent reads; the reader differs, so the rules do. Fork-authored: upstream owns none of these bytes.

## Model-invoked

Model- or user-reachable (rich trigger phrasing so the model can reach for them).

- **[unslop](./unslop/SKILL.md)** cuts the AI tells out of a draft and gives it a voice back. Sets the register first, because the voice advice that saves an essay ruins a config doc. Ships a checker that finds the mechanical tells, so judgment goes to the ones a regex cannot see.
