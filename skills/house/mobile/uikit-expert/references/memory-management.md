...
**Non-escaping closures cannot create retain cycles** because the closure executes immediately and is never stored. This includes `map`, `filter`, `forEach`, `compactMap`, and `UIView.animate`:
...
`Timer.scheduledTimer(timeInterval:target:selector:userInfo:repeats:)` **strongly retains its target**. The RunLoop also retains the Timer independently. This means `deinit` will never fire to invalidate it, creating a deadlock:
...
```swift
// ✅ Combine alternative with automatic cleanup
import Combine
...
**Step 4: Trace the graph.** Click an instance to see a visual graph in the center panel. **Bold arrows are strong references; dashed are weak.** Follow strong arrows to find the cycle, typically a closure context pointing back to your VC, a Timer, or a delegate without `weak`.
...
- [ ] Memory Graph Debugger used periodically; check instance counts after repeated navigation
