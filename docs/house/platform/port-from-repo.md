...
You invoke this by typing `/port-from-repo`, and the agent won't reach for it on its own.
...
It is the step people skip, and the reason ports go wrong. It pushes on four questions: what is the smallest thing that solves our problem, what do we already have that overlaps, which of the source's constraints are we inheriting, and what happens if we do nothing. They routinely cut the scope to a fraction of what the source built.
...
Check before you start. The skill ports an approach rather than an expression, which is the safer footing, but a licence that restricts derivative work is a reason not to proceed, so raise it rather than working around it.
...
- The scope shrank during the challenge phase, so you are building less than the source did.
