...
With iPadOS 16+ **Stage Manager** and resizable windows, apps can encounter arbitrary window sizes. The system still reports Compact or Regular based on the window's width using the same approximate thresholds (~680 pt for iPad). Always use size classes rather than hard-code device idiom checks for layout decisions.
