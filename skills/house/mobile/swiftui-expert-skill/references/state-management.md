...
**Note**: You may want to mark `@Observable` classes with `@MainActor` to ensure thread safety with SwiftUI, unless your project or package uses Default Actor Isolation set to `MainActor`; in that case, the explicit attribute is redundant and can be omitted.
...
Observation tracks reads at the **property** level, not the field level; reading any part of a compound property establishes a dependency on the whole thing. Three common traps and their fixes:
