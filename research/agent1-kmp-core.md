# Kotlin Multiplatform Fundamentals & iOS Integration

**Research Agent 1 of 3** — for a mobile team building shared Kotlin + Compose Multiplatform apps targeting Android and iOS (Swift). Researched 2026-08-06 from the official kotlinlang.org docs plus Touchlab/SKIE material.

**Current stable Kotlin: 2.4.10** (bug-fix release, July 14 2026, on top of Kotlin 2.4.0, June 3 2026). Cadence since 2.0: language releases (2.x.0) every ~6 months, tooling releases (2.x.20) 3 months later, bug-fix releases as needed. Upcoming: 2.4.20 (Sept 2026), 2.5.0 (Dec 2026).

> **Fetch notes:** `get-started.html` and its redirect target returned only a title/redirect page, so getting-started specifics are summarized from the other pages. `native-objc-interop.html` under the `/multiplatform/` path 404s — the working URL is [kotlinlang.org/docs/native-objc-interop.html](https://kotlinlang.org/docs/native-objc-interop.html). All other target pages fetched successfully.
...
Agent skills a team in this area would benefit from:

1. **`kmp-module-setup`** (user-invoked) — scaffold or audit a shared module: targets, default hierarchy, version-catalog entries (Kotlin 2.4.x, AGP, CMP), framework block with `isStatic = true`, `commonTest` wiring. Encodes the compatibility matrix so version bumps are checked, not guessed.
2. **`kmp-ios-integration`** (user-invoked) — decision tree + setup steps for direct integration vs CocoaPods vs SPM vs KMMBridge, including the exact run-script phase, sandboxing gotcha, and migration steps between methods.
3. **`swift-api-surface-review`** (model-invoked) — when editing exported `commonMain` API, review for interop hazards: missing `@Throws`, sealed classes/enums crossing the boundary without SKIE, default args, unconstrained generics, `implementation` deps that need `export()`. Essentially a lint pass over §4's checklist.
4. **`expect-actual-or-interface`** (model-invoked) — guidance skill triggered when adding platform-specific behavior: default to interface + DI, allow expect/actual only for leaf utilities; generates both source-set stubs in the right packages.
5. **`kmp-version-bump`** (user-invoked) — coordinated upgrade of Kotlin/AGP/CMP/SKIE across the catalog, running Android + iOS test suites and the Xcode build to verify; knows about Swift Export's status per release so the team re-evaluates SKIE at each language release.
