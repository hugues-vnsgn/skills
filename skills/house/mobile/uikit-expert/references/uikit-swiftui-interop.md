...
The critical insight: **SwiftUI reuses the underlying UIView instance** even when the `UIViewRepresentable` struct is recreated. The struct is a value type, a description rather than a reference. Any configuration that depends on SwiftUI state must go in `updateUIView`, not `makeUIView`.
...
**Key nuance:** Never push view controllers or present modals inside `updateUIViewController` because it runs on every state change. Navigation actions must be guarded or triggered only by explicit user intent via the coordinator.
...
```swift
// ✅ BEST (iOS 16+): UIHostingConfiguration is lightweight, with no VC overhead
func tableView(_ tableView: UITableView,
               cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath)
