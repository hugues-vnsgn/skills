...
```swift
// ❌ Incorrect: forgetting to save progress; it causes a jump when interrupting mid-flight
case .began:
    animator.pauseAnimation()
...
```swift
// ❌ Incorrect: passing raw points/sec as velocity; it causes extreme overshoot
...
```swift
// ❌ Incorrect: forgetting to update the model; the view snaps back after animation
let animation = CABasicAnimation(keyPath: "cornerRadius")
